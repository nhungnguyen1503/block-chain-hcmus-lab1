...
class RaftServicer(raft_pb2_grpc.RaftServicer):
    ...
    def send_append_entries(self, peer):
        try:
            with grpc.insecure_channel(f'localhost:{peer}') as channel:
                ...
                if response.success:
                    logging.info(f"AppendEntries succeeded for Node {peer}")
                else:
                    logging.warning(f"AppendEntries failed for Node {peer}, Term mismatch")
        except Exception as e:
            logging.error(f"Failed to send AppendEntries to Node {peer}: {e}")

    def RequestVote(self, request, context):
        logging.info(f"RequestVote called on node {self.node_id} with request: {request}")
        with self.lock:
    ...
    def AppendEntries(self, request, context):
        ...
        with self.lock:
            # If the leader's term is greater, update follower's state
            if request.term < self.current_term:
                logging.warning(f"Node {self.node_id} rejected AppendEntries: term {request.term} < current term {self.current_term}")
            ...
            # Ensure log consistency by checking the previous log index and term
            if request.prevLogIndex >= len(self.log) or (request.prevLogIndex >= 0 and self.log[request.prevLogIndex].term != request.prevLogTerm):
                logging.warning(f"Node {self.node_id} rejected AppendEntries: log inconsistency (prevLogIndex={request.prevLogIndex}, prevLogTerm={request.prevLogTerm})")
                return raft_pb2.AppendEntriesResponse(term=self.current_term, success=False)

        logging.info(f"Node {self.node_id} accepted AppendEntries")
